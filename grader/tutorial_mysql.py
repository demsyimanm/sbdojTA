...
db_kunci = MySQLdb.connect(str(sys.argv[4]), str(sys.argv[2]), str(sys.argv[3]), str(sys.argv[5]))
...
def check(submission_id):
    ...
    try:
        ...
        cursor = db.cursor()
        sql = '''select tutorial_id, jawaban from submissiontutorial where id = '''+str(submission_id)
        cursor.execute(sql)
        ...
        for res in results:
            ...
            kunci = '''select jawaban from tutorial where id='''+ str(ques_id)
                ...
                for row_compare in range(row_kunci):
                    for column_compare in range(num_fields):
                        if (hasil[row_compare][column_compare]!=arr_kunci[row_compare][column_compare]):
                            ...
                            flag=1
        if (flag==1): 
            update1 = '''update submissiontutorial set nilai = 0, status = 1 where id = '''+str(sub_id)
            cursor.execute(update1)
            db.commit()
            print 'query '+str(sub_id)+' failed'

        elif (flag==0): 
            update2 = '''update submissiontorial set nilai = 100, status = 1 where id = '''+str(sub_id)
...
def stopwatch(timeout, target, args):
    ...
    db.rollback()
    updateTLE = '''update submissiontutorial set nilai = 101, status = 1 where id = '''+str(sub_id)
...
if __name__ == '__main__':
    try:
        while True:
            ...
            cursor = db.cursor()
            #try:
            sql = '''select id, tutorial_id, users_id, jawaban,status from submissiontutorial where status = 0 having id = min(id)'''
            cursor.execute(sql)
            unchecked = cursor.fetchone()
            sub_id = ''
            ques_id = ''
            ...
            if (tanda==1):
                t = threading.Thread(target=stopwatch, args=(10,check,(str(sub_id),)))
                t.start()
                time.sleep(1)
                t.join()
                print "next"
