...
<div class="ui text" style="margin-bottom:10%;padding-left:5%;padding-right:5%">
	...
	<?php 
	    if (Session::has('status') and Session::get('status') == "exist")
	    {
	       ?>
		        ...
		        <div class="content">
		          ...
		          <p>Mohon memasukkan kategori dengan nama yang berbeda</p>
		        </div>
		      </div>
		    <?php
	    }
    ?>
	  ...
	  <div style="padding-top:3%;padding-bottom:3%">
		  <a class="ui icon teal button" href="{{url('tutorial/add')}}">
			<i class="plus icon"></i>
			Tambah Tutorial
		  </a>
	  </div>
	  <div style="padding:0%;padding-top:0px">
		  <div class="ui blue segment" style="height:80%">
			<table class="ui celled table segment table-hover" id="matkul">
			  ...
			  <tbody>
			  	<?php $i = 1?>
			  	@foreach ($tutorials as $eve)
				    <tr>
				      	<td class="center"><?php echo $i++ ?></td>
			      		<td><a href="{{ URL::to('tutorial/questions/manage/'. $eve->id) }}" >{{ $eve->judul }}</a></td>
			      		<?php
			      		...
			      		<td>
					      	<center>
						      	<a class="ui icon blue button" href="{{ URL::to('tutorial/edit/'. $eve->id) }}">
						        	<i class="pencil icon"></i>
						        	Edit
						      	</a>

						      	<button class="ui icon test red button del" onclick="dele('{{URL::to('tutorial/remove/'. $eve->id) }}')">
